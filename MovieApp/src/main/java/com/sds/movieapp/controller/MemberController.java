...
public class MemberController {
	...
	public ModelAndView naverCallback(HttpServletRequest request, HttpSession session) {
		...
		//신규 회원 가입자인 경우, 회원정보에 sns유형 권한정보도 입력해야 하므로 Member DTO에 이 정보들도 구성해야함
		Member member = new Member();
		member.setUid(id);
		member.setNickname(name);
		member.setEmail(email);
		member.setSns(snsService.selectByName("naver"));
		member.setRole(roleService.selectByName("USER")); //일반회원가입
		...
		//Auth로 들어오더라도 시큐리티를 만나게됨.
		//스프링 시큐리티의 권한 부여를 강제적으로 처리(CustomUserDetails로 부터 자동으로 값을 할당하는 방식이 아니라 개발자가 수동으로 시큐리티에게 정보 주입)
